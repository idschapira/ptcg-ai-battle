"""Local self-play smoke test: N games of RandomAgent vs RandomAgent.
...
Run from the repo root:  python -m src.environment_wrapper.selfplay --games 20
"""
...
def play_one_game(agents: tuple[Agent, Agent], deck0: list[int], deck1: list[int]) -> tuple[int, int]:
    ...
    try:
        for _ in range(MAX_SELECTIONS_PER_GAME):
            ...
            answer = agents[acting_player](obs_dict)
...
def run_selfplay(n_games: int, seed: int = 0) -> SelfPlayStats:
    deck = read_deck_csv()
    stats = SelfPlayStats()
    for game_index in range(n_games):
        agents: tuple[Agent, Agent] = (
            RandomAgent(seed=seed + 2 * game_index),
...
def main() -> None:
    ...
    stats = run_selfplay(args.games, args.seed)
